...
def plot_mission(results):
    ...
    #   Throttle
    # ------------------------------------------------------------------
    #f3 = open(Mission_name + '_throttle.dat', 'w')
    #f3.write('Distance [nm], Throttle\n')
    ...
    for i in range(len(results.segments)):
        ...
        eta_elc = results.segments[i].conditions.propulsion.electric_throttle[:, 0]
        eta_elc_WTP = results.segments[i].conditions.propulsion.electric_throttle_WTP[:, 0]
        axes.plot(distance, eta, 'o-', color='blue', linewidth=1.0, markersize=markersize)
        axes.plot(distance, eta_elc, 'o-', color='red', linewidth=1.0, markersize=markersize)
        axes.plot(distance, eta_elc_WTP, 'o-', color='green', linewidth=1.0, markersize=markersize)
    ...
    for segment in results.segments.values():
        ...
        #             ylabel='TAS [mph]', ylabel_size=ylabel_size, \
        #             title='', title_size=title_size, \
        #             tick_size=tick_size, tick_rotation=tick_rotation, \
        #             grid=grid, minor_ticks=minor_ticks)
        ...
        axes.get_yaxis().get_major_formatter().set_useOffset(False)
        # plot_format(fig, axes, xlabel='Time [mins]', xlabel_size=xlabel_size, \
        #             ylabel='EAS [mph]', ylabel_size=ylabel_size, \
        #             title='', title_size=title_size, \
        #             tick_size=tick_size, tick_rotation=tick_rotation, \
        #             grid=grid, minor_ticks=minor_ticks)
        # axes.set_ylim(70, 110)
    ...
    for segment in results.segments.values():
        ...
        axes.plot( time , percent_mac, 'bo-')
        axes.set_xlabel('Time (min)',axis_font)
        axes.set_ylabel('Percent MAC',axis_font)
        axes.grid(True)
    ...
    axis_font = {'size': '14'}
    fig = plt.figure("Thermal_Management System")
    fig.set_size_inches(10, 8)

    energy_max = np.ones_like(results.segments[0].conditions.propulsion.battery_energy[:,0]) * results.segments[0].conditions.propulsion.battery_energy[0,0]/0.8 # max SoC check todo
    #print(energy_max)
    for segment in results.segments.values():
        time = segment.conditions.frames.inertial.time[:, 0] / Units.min
        altitude = segment.conditions.freestream.altitude[:, 0] / Units.ft
        #heat = segment.conditions.propulsion.heat_load[:, 0] / Units.kW
        heat_vcs = segment.conditions.propulsion.heat_load_vcs[:, 0] / Units.kW
        heat_liquid = segment.conditions.propulsion.heat_load_liquid[:, 0] / Units.kW
        battery_losses = segment.conditions.propulsion.battery_resistive_losses[:, 0] / Units.kW
        battery_power = segment.conditions.propulsion.battery_draw[:, 0] / Units.kW
        ...
        battery_amp_hr = (energy_max/ Units.Wh )/volts

        C_rating       = current/battery_amp_hr

        axes = plt.subplot(4, 1, 1)
        axes.plot(time, -battery_power, 'bo-')
        axes.set_ylabel('Battery Power (kW)', axis_font)
        axes.grid(True)

        axes = plt.subplot(4, 1, 2)
        axes.plot(time, heat_vcs, 'bo-')
        axes.plot(time, heat_liquid, 'ro-')
        axes.set_ylabel('Heat for TMS (kW)', axis_font)
        axes.grid(True)
        ...
        axes.plot(time, battery_losses, 'bo-')
        axes.set_ylabel('Battery Loss (kW)', axis_font)
        axes.set_xlabel('Time (min)', axis_font)
        axes.grid(True)
...
def plot_mission_single(results):
    ...
    segment = results.segments.descent_10
    distance = (segment.conditions.frames.inertial.position_vector[:, 0]) / Units['nautical_mile']
    time = segment.conditions.frames.inertial.time[:, 0] / Units.min
    CLift = segment.conditions.aerodynamics.lift_coefficient[:, 0]
    CDrag = segment.conditions.aerodynamics.drag_coefficient[:, 0]
