...
def results_show(results,vehicle,iteration_setup,first_climb_segment,last_climb_segment,last_descent_segment,first_descent_segment,\
    ...
    plot_propeller_conditions(results, 'bo-')
    #plot_delft_propeller_conditions(results, 'bo-' )
    ...
    print('Wing total CG %.2f m' % ((vehicle.wings.main_wing.origin[0][0])+(vehicle.wings.main_wing.mass_properties.center_of_gravity[0][0])))
    print('Wing origin %.2f m' % ((vehicle.wings.main_wing.origin[0][0]) ))
   # print('MAC origin %.2f m' % ((vehicle.wings.mac.origin[0][0])))
    print('-')
    print('HTP Mass %.2f kg' % vehicle.wings.horizontal_stabilizer.mass_properties.mass)
    print('HTP total CG %.2f m' % ((vehicle.wings.horizontal_stabilizer.origin[0][0])+(vehicle.wings.horizontal_stabilizer.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('VTP Mass %.2f kg' % vehicle.wings.vertical_stabilizer.mass_properties.mass)
    print('VTP total CG %.2f m' % ((vehicle.wings.vertical_stabilizer.origin[0][0])+(vehicle.wings.vertical_stabilizer.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Tail Mass %.2f kg' % (vehicle.wings.horizontal_stabilizer.mass_properties.mass + vehicle.wings.vertical_stabilizer.mass_properties.mass))
    print('-')
    print('ENG Mass %.2f kg' % vehicle.propulsors.network.mass_properties.mass)
    print('Eng total CG %.2f m' % ((vehicle.propulsors.network.origin[0][0]) + (vehicle.propulsors.network.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Propeller Mass %.2f kg' % vehicle.systems.propeller.mass_properties.mass)
    print('Propeller total CG %.2f m' % ((vehicle.systems.propeller.origin[0][0]) + (vehicle.systems.propeller.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Gearbox Mass %.2f kg' % vehicle.systems.gearbox.mass_properties.mass)
    print('Gearbox total CG %.2f m' % ((vehicle.systems.gearbox.origin[0][0]) + (vehicle.systems.gearbox.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Turboshaft Mass %.2f kg' % vehicle.systems.turboshaft.mass_properties.mass)
    print('Turboshaft total CG %.2f m' % ((vehicle.systems.turboshaft.origin[0][0]) + (vehicle.systems.turboshaft.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Total Propulsor Mass %.2f kg' % ((vehicle.systems.turboshaft.mass_properties.mass) + (vehicle.systems.gearbox.mass_properties.mass) + (vehicle.systems.propeller.mass_properties.mass)))
    print('-')
    ...
    print('Main Gear total CG %.2f m' % ((vehicle.landing_gear.main.origin[0][0])+(vehicle.landing_gear.main.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Nose Gear Mass %.2f kg' % (vehicle.landing_gear.nose.mass_properties.mass))
    print('Nose Gear total CG %.2f m' % ((vehicle.landing_gear.nose.origin[0][0])+(vehicle.landing_gear.nose.mass_properties.center_of_gravity[0][0])))
    print('-')
    ...
    print('Control System total CG %.2f m' % ((vehicle.systems.control_systems.origin[0][0])+(vehicle.systems.control_systems.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Electrical System Mass %.2f kg' % vehicle.systems.electrical_systems.mass_properties.mass)
    print('Electrical System total CG %.2f m' % ((vehicle.systems.electrical_systems.origin[0][0])+(vehicle.systems.electrical_systems.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Avionics Mass %.2f kg' % vehicle.systems.avionics.mass_properties.mass)
    print('Avionics total CG %.2f m' % ((vehicle.systems.avionics.origin[0][0])+(vehicle.systems.avionics.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Air Con Mass %.2f kg' % vehicle.systems.air_conditioner.mass_properties.mass)
    print('Air Con total CG %.2f m' % ((vehicle.systems.air_conditioner.origin[0][0]) + (vehicle.systems.air_conditioner.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Hydraulics Mass %.2f kg' % vehicle.systems.hydraulics.mass_properties.mass)
    print('Hydraulics total CG %.2f m' % ((vehicle.systems.hydraulics.origin[0][0]) + (vehicle.systems.hydraulics.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Design Fuel Mass %.2f kg' % vehicle.systems.fuel.mass_properties.mass)
    print('Design Fuel total CG %.2f m' % ((vehicle.systems.fuel.origin[0][0]) + ( vehicle.systems.fuel.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Furnishing Mass %.2f kg' % vehicle.systems.furnishings.mass_properties.mass)
    print('Furnishing total CG %.2f m' % ((vehicle.systems.furnishings.origin[0][0]) + ( vehicle.systems.furnishings.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Payload Mass %.2f kg' % vehicle.mass_properties.payload)
    print('Payload total CG %.2f m' %  ((vehicle.payload.passengers.origin[0][0]) + (vehicle.payload.passengers.mass_properties.center_of_gravity[0][0])))
    print('-')
    print('Oprational Items %.2f kg' % vehicle.systems.optionals.mass_properties.mass)
    print('Oprational Items total CG %.2f m' % ((vehicle.systems.optionals.origin[0][0]) + (vehicle.systems.optionals.mass_properties.center_of_gravity[0][0])))
    print('Crew %.2f kg' % vehicle.systems.crew.mass_properties.mass)
    print('Crew total CG %.2f m' % ((vehicle.systems.crew.origin[0][0]) + (vehicle.systems.crew.mass_properties.center_of_gravity[0][0])))

    print('-')
    print('Oprational Items incl. Crew Mass %.2f kg' % vehicle.mass_properties.operationals)
    print('-')
   # print('Center of Gravity check %.2f m' %  ((1601.03*9.19) + (184.50*20.38) + (270.07*18.81) + (1737.00*8.43) + (2085.71*9.07) + (554.96*10.17) + (115.43*1.86) + (328.25*9.42) + (465.15*12.41) + (988.89 * 1.36) + (431.74* 3.41) + (90.55 * 12.73)+ (2789.98 * 9.93) + (1196.49 * 11.11) + (4560.00 * 10.70) + (1200.31 * 11.11)) / (1601.03+454.57+1737.00+2085.71+670.39+3501.06+4560.00+1200.31))
    ...
    print('xMAC l.e %.2f m == 9.07 m from Reference' % (vehicle.wings.main_wing.origin[0][0] +
                               vehicle.wings.main_wing.aerodynamic_center[0] - 0.25 * vehicle.wings.main_wing.chords.mean_aerodynamic))
