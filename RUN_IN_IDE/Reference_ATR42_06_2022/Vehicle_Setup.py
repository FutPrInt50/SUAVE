...
def vehicle_setup(iteration_setup):
    ...
    vehicle.tag = 'ATR42_reference_aircraft'
    ...
    vehicle.mass_properties.max_payload = 5450.0 * Units.kg #5300.0 * Units.kg
 #  vehicle.mass_properties.max_fuel = 4500.0 * Units.kg
    ...
    # basic parameters
    vehicle.wing_loading = 341.3
    vehicle.reference_area =  float(vehicle.mass_properties.max_takeoff / vehicle.wing_loading)#54.5 * Units['meters**2']
    vehicle.passengers = 48


    vehicle.design_mach_number = 0.48
    vehicle.design_range = iteration_setup.mission_iter.mission_distance #840 * Units.nautical_miles
    ...
    create_tapered_wing(wing, 11.076, vehicle.reference_area * Units['m**2'], span_limit=36 * Units.meter)
    ...
    wing.c_ht =1.065# 1.042#0.9 # twin turboprop
    wing.origin = [[0, 0, 5.]]
    wing.origin[0][0] = vehicle.fuselages.fuselage.lengths.nose + vehicle.fuselages.fuselage.lengths.cabin + 0.705 * vehicle.fuselages.fuselage.lengths.tail #0.64
    ...
    wing.c_vt = 0.1249#0.122#

    wing.origin = [[0, 0, 1.1]]

    wing.origin[0][0] = vehicle.fuselages.fuselage.lengths.nose + vehicle.fuselages.fuselage.lengths.cabin + 0.412 * vehicle.fuselages.fuselage.lengths.tail
    ...
    net.engine_length = 3.5 * Units.meter #length of hole turboshaft including turboshaft, gear and propeller
    net.thrust_angle = 0.0 * Units.deg

    # compute turboshaft areas
    net.areas = Data()
    ...
    turboshaft.origin = [[7.84, 4.05, 0], [7.84, -4.05, 0]]
    turboshaft.turboshaft_length = 1.572 * Units.m #just turboshaft
    turboshaft.specific_power = 1790 / 412 * 1000 * Units.W / Units.kg
    ...
    turboshaft.sfc_scaling_factor =1#1.282 #1#1.16# #1.09#1.55957#1.54814 #1.557145 #ATR 2005 mission
    turboshaft.off_takes = 0.08#0.0425
    ...
    file_name = 'ATR-Feb2023.csv' #PW127-MAX_TET_1385K_Delta_ISA0_low.csv #PW127-MAX_TET_1385K.csv
    ...
    gearbox.origin = [[7.29, 4.05, 0],[7.29, -4.05, 0]]
    ...
    propeller = SUAVE.Components.Energy.Converters.Propeller_Lo_Fid()
    propeller.origin = [[7.015, 4.05, 0],[7.015, -4.05, 0]]

    propeller.tip_radius = 1.965 * Units.meter
    propeller.propulsive_efficiency = 0.83
    propeller.number_of_blades = 6
    propeller.nominal_rpm = 1200 #rpm
...
if __name__ == '__main__':
    ...
    iteration_setup.weight_iter.Design_Payload = 4560. * Units.kg
