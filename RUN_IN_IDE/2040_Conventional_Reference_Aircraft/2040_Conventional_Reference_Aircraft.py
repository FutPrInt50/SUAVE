## 2040_Conventional_Reference_Aircraft.py
# 
...
def base_analysis(vehicle):
    ...
    # ------------------------------------------------------------------
    #  Weights
    mk =0.14 #0.084# for 800 km Design Range # 0.135 for 840 NM Design Range # material mass improvement factor
    ...
    weights.settings.weight_reduction_factors.fuselage = 0.0389+ mk
    ...
    aerodynamics.settings.drag_coefficient_increment.takeoff =  0.01
    aerodynamics.settings.drag_coefficient_increment.climb   =0
    aerodynamics.settings.drag_coefficient_increment.cruise  =0.00005
    aerodynamics.settings.drag_coefficient_increment.descent =0.0000
...
def configs_setup(vehicle):
    ...
    config.propulsors.network.propeller.propulsive_efficiency = 0.4 #0.4 180 0.6
    config.landing_gear.landing_gear_extracted = False
...
if __name__ == '__main__':
    ...
    #iteration_setup.weight_iter.Max_Fuel =  3700. * Units.kg #4100
    iteration_setup.weight_iter.Max_PAX = 5300 * Units.kg

    iteration_setup.sizing_iter.wing_origin = [[8.744, 0, 1.7]] #9.3
    iteration_setup.sizing_iter.factor_engine_power = 1



    iteration_setup.mission_iter.Mission_name = 'Design range mission'
    iteration_setup.mission_iter.mission_distance = 840 * Units.nautical_miles
    iteration_setup.mission_iter.cruise_altitude = 21000
    iteration_setup.mission_iter.cruise_distance = 1225744.8
    iteration_setup.mission_iter.cruise_speed    = 283 * Units.knots
    iteration_setup.mission_iter.reserve_cruise_distance = 50763.32
    iteration_setup.mission_iter.reserve_mission = 1
    ...
    ## while (abs(block_distance*Units['nautical_mile']-mission_distance) > 1.0) and (abs(landing_weight-BOW-PAX) > 1.0):
      ...
      #  print('OEW_Weights: %.1f' % configs.base.mass_properties.operating_empty)
       ...
       # print('Cruise distance: %.1f' % iteration_setup.mission_iter.reserve_cruise_distance)
        ...
        percent_mac_iter = 15 # 14-27 acc. Strohmayer
    ...
    #off_design_mission_calculation_variable_reserve, results = off_design_mission_calculation_variable_reserve(configs.base,mission,"cruise",iteration_setup,5300, 400 * Units.km) #Maximum Range TJM-MJC
...
# ATR42 from 2023-05-17 12:49:40 (Deviation=1.00)
    ...
    fom.settings.trl.electric_motors = 9.
    fom.settings.trl.wing_tip_propulsion = 6.
    fom.settings.trl.batteries = 9.
    ...
    fom.settings.main_wing_comp = True
    fom.settings.fuselage_comp = False
    fom.settings.empennage_comp = True
   ...
   # results_show(results,configs.base,iteration_setup,first_climb_segment,last_climb_segment,last_descent_segment,first_descent_segment,first_reserve_climb_segment,last_reserve_descent_segment,first_reserve_descent_segment,last_reserve_climb_segment,climb_segments)
    ...
    absolute_results_folder_path = os.path.abspath("../_results")
    # if configs.base.propulsors.network.turboshaft.p3t3_method:
    #     results = gas_turbine_calculate_emission(results, configs.base)
