...
def plot_altitude_sfc_weight(results, line_color = 'bo-', save_figure = False, save_filename = "Altitude_SFC_Weight" , file_type = ".png"):
    ...
    for segment in results.segments.values(): 
        ...
        axes.set_ylabel('TAS (kts)', axis_font)
        set_axes(axes)

        axes = plt.subplot(4, 1, 2)
        axes.plot(time, EAS / Units.kts, line_color)
        axes.set_ylabel('EAS (kts)', axis_font)
        set_axes(axes)

        axes = plt.subplot(4, 1, 3)
        axes.plot(time, CAS / Units.kts, line_color)
        axes.set_ylabel('CAS (kts)', axis_font)
        # axes.set_ylim(np.min(CAS)*0.9,np.max(CAS)*1.1)
        set_axes(axes)
...
def plot_delft_propeller_conditions(results, propellerTag, line_color='bo-', line_color_wtp='ro-', save_figure=False, save_filename="Propeller",
                              file_type=".png"):
    """This plots the propeller performance
    ...
    for segment in results.segments.values():
        time    = segment.conditions.frames.inertial.time[:, 0] / Units.min
        rpm     = segment.conditions.propulsion['rpm_propeller'][:,0]
        #thrust  = segment.conditions.propulsion['thrust_propeller'][:,0]
        beta    = segment.conditions.propulsion['beta_propeller'][:, 0]
        etap    = segment.conditions.propulsion['eta_propeller'][:, 0]
        J       = segment.conditions.propulsion['j_propeller'][:, 0]
        Cp      = segment.conditions.propulsion['cp_propeller'][:, 0]
        Ct      = segment.conditions.propulsion['ct_propeller'][:,0]

        useWTP = False
        if 'rpm_propellerWTP' in segment.conditions.propulsion.keys():
            useWTP = True
            rpm_wtp = segment.conditions.propulsion['rpm_propellerWTP'][:, 0]
            #thrust_wtp = segment.conditions.propulsion['thrust_propellerWTP'][:, 0]
            beta_wtp = segment.conditions.propulsion['beta_propellerWTP'][:, 0]
            etap_wtp = segment.conditions.propulsion['eta_propellerWTP'][:, 0]
            J_wtp = segment.conditions.propulsion['j_propellerWTP'][:, 0]
            Cp_wtp = segment.conditions.propulsion['cp_propellerWTP'][:, 0]
            Ct_wtp = segment.conditions.propulsion['ct_propellerWTP'][:, 0]

        # axes1 = plt.subplot(2, 3, 1)
        # axes1.plot(time, thrust, line_color, label='Turboprop')
        # if useWTP == True:  axes1.plot(time, thrust_wtp, line_color_wtp, label='WTP')
        # axes1.set_ylabel('Thrust (N)', axis_font)
        # set_axes(axes1)
        # if i == 0:
        #     axes1.legend(loc='upper center')

        axes1 = plt.subplot(2, 3, 1)
        axes1.plot(time, beta, line_color, label='Turboprop')
        if useWTP == True:  axes1.plot(time, beta_wtp, line_color_wtp, label='WTP')
        axes1.set_ylabel('Beta [deg]', axis_font)
        set_axes(axes1)
        if i == 0:
            axes1.legend(loc='upper center')

        axes2 = plt.subplot(2, 3, 2)
        axes2.plot(time, rpm, line_color)
        if useWTP == True: axes2.plot(time, rpm_wtp, line_color_wtp)
        axes2.set_ylabel('RPM', axis_font)
        set_axes(axes2)

        axes3 = plt.subplot(2, 3, 3)
        axes3.plot(time, etap, line_color)
        if useWTP == True: axes3.plot(time, etap_wtp, line_color_wtp)
        axes3.set_xlabel('Time (mins)', axis_font)
        axes3.set_ylabel('Eta', axis_font)
        axes3.set_ylim([0., 1.])
        set_axes(axes3)

        axes4 = plt.subplot(2, 3, 4)
        axes4.plot(time, J, line_color)
        if useWTP == True: axes4.plot(time, J_wtp, line_color_wtp)
        axes4.set_ylabel('J', axis_font)
        set_axes(axes4)

        axes5 = plt.subplot(2, 3, 5)
        axes5.plot(time, Cp, line_color)
        if useWTP == True: axes5.plot(time, Cp_wtp, line_color_wtp)
        axes5.set_xlabel('Time (mins)', axis_font)
        axes5.set_ylabel('Power Coefficient', axis_font)
        set_axes(axes5)

        axes6 = plt.subplot(2, 3, 6)
        axes6.plot(time, Ct, line_color)
        if useWTP == True: axes6.plot(time, Ct_wtp, line_color_wtp)
        axes6.set_xlabel('Time (mins)', axis_font)
        axes6.set_ylabel('Thrust Coefficient', axis_font)
        set_axes(axes6)
...
def plot_aerodynamic_coefficients_custom(results, save_figure=False, save_filename="Aerodynamic_Coefficients"):
    markersize = 3
...
def plot_drag_components_custom(results, save_figure=False, save_filename="Drag_Components"):
    markersize = 3
...
def plot_detailed_parasite_drag(results, save_figure=False, save_filename="Detailed_Parasite_Drag"):
    markersize = 3
...
def plot_throttle(results, save_figure=False, save_filename="Throttle"):
    markersize = 3
...
def plot_mission_profile(results, save_figure=False, save_filename="Mission_Profile"):
    markersize = 3
...
def plot_altitude_sfc_weight_custom(results, save_figure=False, save_filename="Altitude_SFC_weight"):
    axis_font = {'size': '14'}
...
def plot_altitude_power(results, vehicle, save_figure=False, save_filename="Altitude_Power"):
    axis_font = {'size': '14'}
    ...
    for segment in results.segments.values():
        ...
        TC = thrust / vehicle.propulsors.network.number_of_engines / (
                    rho * segment.conditions.freestream.velocity[:, 0] ** 2 * (
                        vehicle.propulsors.network.propeller.tip_radius * 2) ** 2)
...
def plot_stability(results, save_figure=False, save_filename="Stability"):
    axis_font = {'size': '14'}
